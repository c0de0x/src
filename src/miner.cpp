        ...
        if (pblock->nVersion < 7) {
            if (fZerocoinActive) {
                ...
                if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                    //For the period before v2 activation, zSmnc will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zSmnc key failed \n", __func__);
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("SIMPLEMASTERNODEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            ...
            pwalletMain->zsmncTracker->ListMints(true, true, true); //update the state
        }
        return error("SIMPLEMASTERNODEMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("SIMPLEMASTERNODEMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SIMPLEMASTERNODEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("SIMPLEMASTERNODEMiner exception");
    } catch (...) {
        LogPrintf("SIMPLEMASTERNODEMiner exception");
    }

    LogPrintf("SIMPLEMASTERNODEMiner exiting\n");
}
