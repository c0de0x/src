...
#include "qt/smnc/forms/ui_addresseswidget.h"
#include "qt/smnc/addresslabelrow.h"
#include "qt/smnc/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(SIMPLEMASTERNODEGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a SIMPLEMASTERNODE address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress smncAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(smncAdd.Get(), label.toUtf8().constData(),
                smncAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
