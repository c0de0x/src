...
*Setup instructions for a gitian build of SMNC Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SIMPLEMASTERNODE
...
to the SIMPLEMASTERNODE GitHub release page.
...
- [Building SMNC Coin](#building-smnc-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/smnc-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/smnc-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/smnc-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/smnc-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/smnc-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/smnc-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/SimpleMasternode/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@smnc.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/smnc-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/smnc-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/smnc-osx-*-build.assert
```
