...
bool CZSMNCControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSMNCControlDialog::COLUMN_DENOMINATION || column == ZSMNCControlDialog::COLUMN_VERSION || column == ZSMNCControlDialog::COLUMN_CONFIRMATIONS)
    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zSmnc Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZSMNC->setProperty("cssClass", "text-main-purple");
    ui->labelZSMNC_int->setProperty("cssClass", "text-main-purple");
...
void ZSMNCControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSMNCControlWidgetItem* itemDenom(new CZSMNCControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSMNCControlWidgetItem *itemMint = new CZSMNCControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zsmncTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsmncTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSmnc.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSmnc seed used to mint this zSmnc is not the same as currently hold in the wallet";
            else
