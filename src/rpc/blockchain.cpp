...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zsmncObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zsmncObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zsmncObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZSmnc()) ? "Not available" : std::to_string(
...
UniValue getblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"zSmncsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zSmnc denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zSmnc denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zSmnc denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zSmnc denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zSmnc denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zSmnc denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zSmnc denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zSmnc denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zSmnc denominations\n"
            "  },\n"
            "  \"CoinStake\" :\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZSmnc_FAILED_ACCUMULATOR_INITIALIZATION);
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  }\n"
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (zSmnc excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (zSmnc mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (zSmnc mints included) over block range\n"
