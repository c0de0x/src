...
#include "qt/smnc/topbar.h"
#include "qt/smnc/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/smnc/lockunlock.h"
#include "qt/smnc/qtutils.h"
#include "qt/smnc/receivedialog.h"
...
TopBar::TopBar(SIMPLEMASTERNODEGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopSMNC, ui->labelAmountTopzSMNC}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSMNC, ui->labelAmountzSMNC}, "amount-topbar");
    setCssProperty({ui->labelPendingSMNC, ui->labelPendingzSMNC, ui->labelImmatureSMNC, ui->labelImmaturezSMNC}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzSMNC->setVisible(false);
    ui->labelAmountzSMNC->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzSMNC->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezSMNC->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("SIMPLEMASTERNODE")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount smncAvailableBalance = balance + delegatedBalance;

    // zSmnc Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalSMNC = GUIUtil::formatBalance(smncAvailableBalance, nDisplayUnit);
    QString totalzSMNC = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopSMNC->setText(totalSMNC);
    ui->labelAmountTopzSMNC->setText(totalzSMNC);

    // Expanded
    ui->labelAmountSMNC->setText(totalSMNC);
    ui->labelAmountzSMNC->setText(totalzSMNC);

    ui->labelPendingSMNC->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzSMNC->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureSMNC->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezSMNC->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
