...
#include "qt/smnc/dashboardwidget.h"
#include "qt/smnc/forms/ui_dashboardwidget.h"
#include "qt/smnc/sendconfirmdialog.h"
#include "qt/smnc/txrow.h"
...
DashboardWidget::DashboardWidget(SIMPLEMASTERNODEGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of Smnc staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareSMNC, "square-chart-smnc");
    setCssProperty(ui->labelSquarezSMNC, "square-chart-zsmnc");
    setCssProperty(ui->labelSMNC, "text-chart-smnc");
    setCssProperty(ui->labelZsmnc, "text-chart-zsmnc");
    ui->labelZsmnc->setVisible(false);
    ui->labelSquarezSMNC->setVisible(false);
    ...
    ui->labelAmountZsmnc->setText("0 zSmnc");
    ui->labelAmountZsmnc->setVisible(false);
    ui->labelAmountSMNC->setText("0 Smnc");
    setCssProperty(ui->labelAmountSMNC, "text-stake-smnc-disable");
    setCssProperty(ui->labelAmountZsmnc, "text-stake-zsmnc-disable");
    ...
    ui->btnHowTo->setText(tr("How to get Smnc"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isSMNC = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZSmnc;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair Smnc, zSmnc
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            smnc = (pair.first != 0) ? pair.first / 100000000 : 0;
            zsmnc = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalSMNC += pair.first;
            chartData->totalZsmnc += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesSMNC.append(smnc);
        chartData->valueszSMNC.append(zsmnc);

        int max = std::max(smnc, zsmnc);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("Smnc");
    set1 = new QBarSet("zSmnc");
    ...
    set0->append(chartData->valuesSMNC);
    set1->append(chartData->valueszSMNC);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalSMNC > 0 || chartData->totalZsmnc > 0) {
        setCssProperty(ui->labelAmountSMNC, "text-stake-smnc");
        setCssProperty(ui->labelAmountZsmnc, "text-stake-zsmnc");
    } else {
        setCssProperty(ui->labelAmountSMNC, "text-stake-smnc-disable");
        setCssProperty(ui->labelAmountZsmnc, "text-stake-zsmnc-disable");
    }
    forceUpdateStyle({ui->labelAmountSMNC, ui->labelAmountZsmnc});
    ui->labelAmountSMNC->setText(GUIUtil::formatBalance(chartData->totalSMNC, nDisplayUnit));
    ui->labelAmountZsmnc->setText(GUIUtil::formatBalance(chartData->totalZsmnc, nDisplayUnit, true));
