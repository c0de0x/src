...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    ...
    if (settings.contains("nAnonymizeSimpleMasternodeAmount") || reset)
        SoftSetArg("-anonymizesmncamount", settings.value("nAnonymizeSimpleMasternodeAmount").toString().toStdString());
