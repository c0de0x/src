...
The build script we'll be using is contained in the SIMPLEMASTERNODE github repository ([contrib/gitian-build.py](https://github.com/SimpleMasternode/smnc/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the SIMPLEMASTERNODE repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/SimpleMasternode/SIMPLEMASTERNODE/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, smnc-detached-sigs, and smnc GitHub repos.
