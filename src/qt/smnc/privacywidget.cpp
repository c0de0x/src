...
#include "qt/smnc/privacywidget.h"
#include "qt/smnc/forms/ui_privacywidget.h"
#include "qt/smnc/qtutils.h"
#include "guiutil.h"
#include "qt/smnc/denomgenerationdialog.h"
#include "qt/smnc/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(SIMPLEMASTERNODEGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zSmnc anonymizes your Smnc by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zSmnc or convert back to Smnc"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 Smnc ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zSmnc");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zSmnc");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zSmnc");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zSmnc");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zSmnc");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zSmnc");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zSmnc");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zSmnc");
    ...
    // Only Convert to Smnc enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzSmnc->setTitleClassAndText("btn-title-grey", "Total 0 zSmnc");
    ui->btnTotalzSmnc->setSubTitleClassAndText("text-subtitle", "Show denominations of zSmnc owned.");
    ui->btnTotalzSmnc->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select Smnc outputs to mint into zSmnc.");
    ...
    connect(ui->btnTotalzSmnc, SIGNAL(clicked()), this, SLOT(onTotalZsmncClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zSmnc");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of Smnc to mint into zSmnc"));
    }else{
        btnText = tr("Convert back to Smnc");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zSmnc to convert back into Smnc"));
    }
...
void PrivacyWidget::onTotalZsmncClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzSmnc->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzSmnc->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zSmnc is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zSmnc, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zSmnc").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zSmnc minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZsmnc(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zSmnc converted back to Smnc"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzSmnc->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
