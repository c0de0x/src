...
/**
 * SIMPLEMASTERNODE RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZsmncSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzsmncseed(const UniValue& params, bool fHelp);
extern UniValue getzsmncseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzsmnc(const UniValue& params, bool fHelp);
extern UniValue dzsmncstate(const UniValue& params, bool fHelp);
