...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zSmnc in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zsmncTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zsmncTracker->ListMints(true, true, true);
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if (GetBoolArg("-smncstake", true) && !fPrecompute) {
    ...
    /* Disable zSmnc Staking
    //zSmnc
    if ((GetBoolArg("-zsmncstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zSmnc set once per update interval
        bool fUpdate = false;
        ...
        std::set<CMintMeta> setMints = zsmncTracker->ListMints(true, true, fUpdate);
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZSMNCStake> input(new CZSMNCStake(meta.denom, meta.hashStake));
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 Smnc collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 Smnc collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 Smnc.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 Smnc.");
                    } else {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zsmncTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zsmncTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zsmncTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZSmnc_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZSmnc_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZSmnc_TXMINT_GENERAL);
        return false;
    }
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZSmnc_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZSmnc_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZSmnc_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZSmncModule::createInput(in, mint, hashTxOut, spendVersion)) {
                receipt.SetStatus(_("Cannot create public spend input"), ZSmnc_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zSmnc public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zSmnc used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zsmncTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
...
std::string CWallet::GetUniqueWalletBackupName(bool fzsmncAuto) const
{
...
void CWallet::ZSMNCBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozsmncbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zsmncbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zsmncbackuppath", ""));
...
{
    // We don't even need to worry about this code.. no zSmnc.
    /*
    LogPrintf("Precomputer started\n");
    RenameThread("smnc-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zsmncTracker->GetSpendCache(serialHash);
...
void CWallet::setZWallet(CzSmncWallet* zwallet)
{
    zwalletMain = zwallet;
    zsmncTracker = std::unique_ptr<CzSmncTracker>(new CzSmncTracker(strWalletFile));
}

CzSmncWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
void CWallet::setZSMNCAutoBackups(bool fEnabled)
{
