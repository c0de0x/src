...
#include "qt/smnc/send.h"
#include "qt/smnc/forms/ui_send.h"
#include "qt/smnc/addnewcontactdialog.h"
#include "qt/smnc/qtutils.h"
#include "qt/smnc/sendchangeaddressdialog.h"
#include "qt/smnc/optionbutton.h"
#include "qt/smnc/sendconfirmdialog.h"
#include "qt/smnc/myaddressrow.h"
#include "qt/smnc/guitransactionsutils.h"
...
SendWidget::SendWidget(SIMPLEMASTERNODEGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("Smnc");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zSmnc");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (Smnc)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a SIMPLEMASTERNODE address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 Smnc");
    ...
    setCssProperty(coinIcon, "coin-icon-smnc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onSmncSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onSmncSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send Smnc");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zSmnc");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZsmnc = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZsmnc));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZsmnc ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendSMNC = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendSMNC ? AskPassphraseDialog::Context::Send_Smnc : AskPassphraseDialog::Context::Send_zSmnc, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendSMNC) ? send(recipients) : sendZsmnc(recipients)) {
...
bool SendWidget::sendZsmnc(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zSmnc is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZsmnc(
        ...
        inform(tr("zSmnc transaction sent!"));
        ...
        if (receipt.GetStatus() == ZSmnc_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zSmnc require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zSmnc transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isSmnc){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any Smnc to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zSMNCControl->exec();
            ui->btnCoinControl->setActive(!ZSMNCControlDialog::setSelectedMints.empty());
            zSMNCControl->deleteLater();
        } else {
            inform(tr("You don't have any zSmnc in your balance to select."));
        }
    }
}
...
void SendWidget::onSmncSelected(bool _isSmnc){
    isSmnc = _isSmnc;
    setCssProperty(coinIcon, _isSmnc ? "coin-icon-smnc" : "coin-icon-zsmnc");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress smncAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(smncAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(smncAdd.Get(), dialog->getLabel().toStdString(), "send")) {
