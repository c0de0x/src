...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZsmnc){
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZsmnc)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZsmnc);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SIMPLEMASTERNODE address (e.g. %1)").arg("A7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no SIMPLEMASTERNODE: URI
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SIMPLEMASTERNODE.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SIMPLEMASTERNODE.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        optionFile << "Name=SIMPLEMASTERNODE\n";
