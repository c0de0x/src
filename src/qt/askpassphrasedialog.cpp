...
#include "qt/smnc/loadingdialog.h"
    ...
    case Mode::Encrypt: {
        ...
        if (ret) {
            if (newpass1 == newpass2) {
                ...
                SIMPLEMASTERNODEGUI* window = static_cast<SIMPLEMASTERNODEGUI*>(parentWidget());
...
{
    SIMPLEMASTERNODEGUI* gui = static_cast<SIMPLEMASTERNODEGUI*>(parentWidget());
...
{
    hide();
    static_cast<SIMPLEMASTERNODEGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("SIMPLEMASTERNODE will close now to finish the encryption process. "
